...
public interface TransactionProjection2 {
	...
	@Value("#{new by.kislyakoff.HomeBudgetApp.dto.projections.Acc1Projection(target.acc1Name, target.acc1CurrencyCode, target.acc1Balance)}")
	Acc1Projection getAcc1();
	
	@Value("#{target.acc2Name != null ? new by.kislyakoff.HomeBudgetApp.dto.projections.Acc1Projection(target.acc2Name, target.acc2CurrencyCode, target.acc2Balance) : null}")
