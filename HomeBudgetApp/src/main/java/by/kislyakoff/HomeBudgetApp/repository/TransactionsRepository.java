...
//      List<TransactionProjection> findByAcc1PersonIdOrderByDateDescTest1(@Param("personId")int personId);
        ...
        Page<TransactionProjection> findByAcc1PersonIdOrderByDateDesc_WithSearchPaginated(
                                        @Param("personId")int personId, @Param("seek") String seek, Pageable pageable);
        ...
        @Query(value = "SELECT t.id AS id, " +
                           ...
                           "LEFT OUTER JOIN category cat ON t.category_id=cat.id " +
                           "WHERE per.id= :personId ORDER BY t.transaction_date DESC", nativeQuery = true)
        Page<TransactionProjection> findByAcc1PersonIdOrderByDateDescPaginated(
                                                                                        @Param("personId")int personId, Pageable pageable);
